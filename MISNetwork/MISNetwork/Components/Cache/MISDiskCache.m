...
static int64_t _MISDiskSpaceFree()
{
    NSError* error = nil;
...
- (void)_trimToFreeDiskSpace:(NSUInteger)targetFreeDiskSpace
{
    ...
    if (totalBytes <= 0)
        return;
    int64_t diskFreeBytes = _MISDiskSpaceFree();
    if (diskFreeBytes < 0)
        return;
...
- (instancetype)init
{
    @throw [NSException exceptionWithName:@"MISDiskCache init error" reason:@"MISDiskCache must be initialized with a path. Use 'initWithPath:' or 'initWithPath:inlineThreshold:' instead." userInfo:nil];
             ...
             inlineThreshold:(NSUInteger)threshold
{
    ...
    MISKVStorageType type;
    if (threshold == 0) {
        type = MISKVStorageTypeFile;
    }
    else if (threshold == NSUIntegerMax) {
        type = MISKVStorageTypeSQLite;
    }
    else {
        type = MISKVStorageTypeMixed;
    }

    MISKVStorage* kv = [[MISKVStorage alloc] initWithPath:path type:type];
    if (!kv)
        return nil;
    ...
    _queue = dispatch_queue_create("com.misnetwork.cache.disk", DISPATCH_QUEUE_CONCURRENT);
...
- (id<NSCoding>)objectForKey:(NSString*)key
{
    if (!key)
        return nil;
    Lock();
    MISKVStorageItem* item = [_kv getItemForKey:key];
    Unlock();
    if (!item.value)
        return nil;
    ...
    if (object && item.extendedData) {
        [MISDiskCache setExtendedData:item.extendedData toObject:object];
    }
    return object;
}
...
- (void)setObject:(id<NSCoding>)object forKey:(NSString*)key
{
    ...
    NSData* extendedData = [MISDiskCache getExtendedDataFromObject:object];
    NSData* value = nil;
    ...
    if (!value)
        return;
    NSString* filename = nil;
    if (_kv.type != MISKVStorageTypeSQLite) {
