...
@implementation MISCache

- (instancetype)init
{
    NSLog(@"Use \"initWithName\" or \"initWithPath\" to create MISCache instance.");
...
- (instancetype)initWithPath:(NSString*)path
{
    if (path.length == 0)
        return nil;
    MISDiskCache* diskCache = [[MISDiskCache alloc] initWithPath:path];
    if (!diskCache)
        return nil;
    NSString* name = [path lastPathComponent];
    MISMemoryCache* memoryCache = [MISMemoryCache new];
