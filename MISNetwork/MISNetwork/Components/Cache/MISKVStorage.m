...
@implementation MISKVStorageItem
@end

@implementation MISKVStorage {
...
- (MISKVStorageItem*)_dbGetItemFromStmt:(sqlite3_stmt*)stmt excludeInlineData:(BOOL)excludeInlineData
{
    int i = 0;
    ...
    MISKVStorageItem* item = [MISKVStorageItem new];
    if (key)
...
- (MISKVStorageItem*)_dbGetItemWithKey:(NSString*)key excludeInlineData:(BOOL)excludeInlineData
{
...
- (NSMutableArray*)_dbGetItemWithKeys:(NSArray*)keys excludeInlineData:(BOOL)excludeInlineData
{
    ...
    do {
        result = sqlite3_step(stmt);
        if (result == SQLITE_ROW) {
            MISKVStorageItem* item = [self _dbGetItemFromStmt:stmt excludeInlineData:excludeInlineData];
            if (item)
...
- (instancetype)init
{
    @throw [NSException exceptionWithName:@"MISKVStorage init error" reason:@"Please use the designated initializer and pass the 'path' and 'type'." userInfo:nil];
    return [self initWithPath:nil type:MISKVStorageTypeFile];
}

- (instancetype)initWithPath:(NSString*)path type:(MISKVStorageType)type
{
    if (path.length == 0 || path.length > kPathLengthMax) {
        NSLog(@"YYKVStorage init error: invalid path: [%@].", path);
        return nil;
    }
    if (type > MISKVStorageTypeMixed) {
    ...
    if (![[NSFileManager defaultManager] createDirectoryAtPath:path
                                   withIntermediateDirectories:YES
                                                    attributes:nil
                                                         error:&error] || ![[NSFileManager defaultManager] createDirectoryAtPath:[path stringByAppendingPathComponent:kDataDirectoryName]
                                                                                                     withIntermediateDirectories:YES
                                                                                                                      attributes:nil
                                                                                                                           error:&error] || ![[NSFileManager defaultManager] createDirectoryAtPath:[path stringByAppendingPathComponent:kTrashDirectoryName]
                                                                                                                                                                       withIntermediateDirectories:YES
                                                                                                                                                                                        attributes:nil
                                                                                                                                                                                             error:&error]) {
...
- (BOOL)saveItemWithKey:(NSString*)key value:(NSData*)value filename:(NSString*)filename extendedData:(NSData*)extendedData
{
    if (key.length == 0 || value.length == 0)
        return NO;
    if (_type == MISKVStorageTypeFile && filename.length == 0) {
        return NO;
    }
    ...
    if (filename.length) {
        ...
        if (_type != MISKVStorageTypeSQLite) {
...
- (BOOL)removeItemForKey:(NSString*)key
{
    if (key.length == 0)
        return NO;
    switch (_type) {
    case MISKVStorageTypeSQLite: {
        return [self _dbDeleteItemWithKey:key];
    } break;
    case MISKVStorageTypeFile:
    case MISKVStorageTypeMixed: {
        NSString* filename = [self _dbGetFilenameWithKey:key];
        if (filename) {
            [self _fileDeleteWithName:filename];
        }
...
- (BOOL)removeItemsToFitSize:(int)maxSize
{
    ...
    do {
        ...
        for (MISKVStorageItem* item in items) {
            if (total > maxSize) {
                if (item.filename) {
...
- (MISKVStorageItem*)getItemForKey:(NSString*)key
{
    if (key.length == 0)
        return nil;
    MISKVStorageItem* item = [self _dbGetItemWithKey:key excludeInlineData:NO];
    if (item) {
...
- (MISKVStorageItem*)getItemInfoForKey:(NSString*)key
{
    if (key.length == 0)
        return nil;
    MISKVStorageItem* item = [self _dbGetItemWithKey:key excludeInlineData:YES];
    return item;
}
