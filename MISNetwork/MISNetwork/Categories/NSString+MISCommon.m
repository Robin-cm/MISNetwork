...
#import "NSData+MISCommon.h"
#import <UIKit/UIKit.h>
#import "NSNumber+MISCommon.h"

@implementation NSString (MISCommon)
...
- (BOOL)containsEmojiForSystemVersion:(float)systemVersion
{
    ...
    if ([self rangeOfCharacterFromSet:maxSet].location == NSNotFound)
        return NO;

    // 2. If the emoji can be detected by the minimum charset, return 'YES' directly.
    if ([self rangeOfCharacterFromSet:((systemVersion < 8.3) ? minSetOld : minSet8_3)].location != NSNotFound)
        return YES;
